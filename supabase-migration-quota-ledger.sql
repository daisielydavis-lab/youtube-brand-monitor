...
create or replace function reserve_youtube_search_quota(
...
begin
  ...
  if p_dry_run then
    select search_calls_used, hard_exhausted into total, he
      from youtube_quota_usage
     where quota_period_date = period and api_key_id = p_key;
    if total + 1 <= p_hard_budget and not he then
      return query select true, 'ok', total, period, reset_ts;
    end if;
    return query select false, case when he then 'hard_exhausted' else 'budget' end, total, period, reset_ts;
  end if;
  ...
  update youtube_quota_usage
     set search_calls_used = search_calls_used + 1,
         ...
         updated_at = now()
   where quota_period_date = period
     and api_key_id = p_key
     and not hard_exhausted
     and search_calls_used + 1 <= p_hard_budget
   returning search_calls_used into u;

  if u is not null then
    return query select true, 'ok', u, period, reset_ts;
  end if;

  select hard_exhausted into he
    from youtube_quota_usage
   where quota_period_date = period and api_key_id = p_key;

  if he then
    return query select false, 'hard_exhausted', 0, period, reset_ts;
  else
    return query select false, 'budget', 0, period, reset_ts;
  end if;
end;
$$;

-- ── 读当前期状态（惰性建行，不递增）── hydrate / dashboard / readiness 共用
create or replace function get_youtube_quota_status(p_key text default 'primary')
returns table (
